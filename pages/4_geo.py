...
# 지도시각화1 - st.map
seoulpop['pop2'] = seoulpop['pop'].apply(lambda x: x / 7500)
print(seoulpop['pop2'])
...
fig = px.scatter_mapbox(seoulpop, lat='lat', lon='lon', size='pop2', color='pop2',
                        color_continuous_scale=px.colors.sequential.Rainbow, mapbox_style='open-street-map',
...
# 지도시각화3 - 동적 시각화
option1 = st.selectbox('보고싶은 인구현황을 선택하세요', ['구별 총 인구수', '구별 총내국인수', '구별 총외국인수'])
...
fig = px.scatter_mapbox(seoulpop, lat='lat', lon='lon', size='pop2', color='pop2',
                        color_continuous_scale=optclor, mapbox_style='open-street-map',
                        hover_name='gu',
...
# 지도시각화4 - 동적 시각화
option1 = st.selectbox('보고싶은 구를 선택하세요', seoulpop['gu'].tolist())
optcols = 'pop' if option1 == '구별 총인구수' else \
          'korpop' if option1 == '구별 총내국인수' else 'forepop'
